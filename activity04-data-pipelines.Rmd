...
**Response**: when messages=false, it takes out information that may not be important to the reader. When messages=true, you get information on conflicts adm the packages that are attached. 
...
```{r}
college_grads_top <- college_recent_grads
arrange(college_grads_top, unemployment_rate)
```
...
### Which major has the lowest unemployment rate?
Mathematics has the lowest unemployment rate. It is tied with education, biology and life sciences, agriculture and natural resources,  and industrial arts and consumer services.
...
**Response**: In SAS, you would have to modifiy the data step/ proc print. 
...
arrange(rearrange_college, unemployment_rate)
```
...
arrange(lowest_unemploy, unemployment_rate)
select(lowest_unemploy, rank, major, unemployment_rate)
```
